...
tot_year <- y %>%
  ...
  summarise(stock = sum(exp(value)))


plot(density(tot_year$stock[tot_year$year==2018]))
...
table_name <- paste0(folder_name, model_name,"_tot_y.csv")
...
plot_name <- paste0("./figures/",model_name,"_total.png")
...
imm <- imm_y %>%
  ...
  mutate(source_name = ifelse(source_name == "Eurostat", "Eurostat", source_name),
         ...
         source_name = source,
         source = ifelse(source_name == "eurostat", "Eurostat", source),
         source = ifelse(source_name == "fbdau", "Facebook DAU", source),
         source = ifelse(source_name == "fbmau", "Facebook MAU", source),
         source = ifelse(source_name == "Estimate", "Estimate", source),
         source = ifelse(source_name == "LFS", "LFS", source),
         source = ifelse(source_name == "census", "Census", source),
         source = fct_relevel(source, "Estimate"),
...
table_name <- paste0(folder_name, model_name,"imm_y.csv")
...
table_name <- paste0(folder_name, model_name,"emi_y.csv")
...
table_name <- paste0(folder_name, model_name,"_bilat_y.csv")
