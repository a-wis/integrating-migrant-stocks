# read and prepare data for models 
# Sensitivity 3-5
...
corrname4 <- expand.grid(dest = sort(eu_countries$name), orig = sort(eu_countries$name), year = seq(2011,2019, by = 1)) %>% 
...
d6.6 <- left_join(corrname4, d6.9) %>%
  ...
  left_join(d6.4) %>% 
  left_join(d6.5) %>% 
  mutate(corridor = as.numeric(corridor_name))
...
d4 <- read.csv("./data/LFS/LFS_corrected_full_corridors.csv") %>% 
...
# write.csv(dinput, "./Paper/data_july19/input_estimate_eurostat_5per.csv", row.names = F)
...
# save(d, file = "./Paper/data_july19/datalist_estimate_eu16_March_June.Rdata")
##
## jags run
##
...
m <- jags.parfit(cl = cl,   
                  ...
                  thin=1000,
                  data = d, model = "./Paper/m_update_2019_oldfbcovgr_prec.txt", 
...
save(m, file = "W:/Users/dyildiz/Desktop/m_estimate_eu_5per.RData", compress = "xz")
